...
| 5 | **Click Launch on Open WebUI** | Open WebUI's UI renders inside the dashboard | **Unverified** — the loopback defect is fixed: `ClientViewer.tsx:386-393` frames the same-origin `/app/<deviceId>/<serviceId>/` path behind the Service Worker (Phase 4), not B's own `127.0.0.1`. Expect to get as far as Open WebUI's sign-in screen and no further, because login is blocked ([`tunnel-proxy-spec.md`](./tunnel-proxy-spec.md) §5.6.2, [#72](https://github.com/lem-app/lem/issues/72)). Not yet run from a second machine. [#6](https://github.com/lem-app/lem/issues/6) |
