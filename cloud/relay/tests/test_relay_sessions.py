...
from tests.conftest import expect_closed, make_grant, wait_until
...
def test_paired_devices_forward_frames_both_ways(client: TestClient) -> None:
    ...
    with client.websocket_connect(f"/relay/{SESSION}?token={browser_grant}") as browser:
        assert wait_until(lambda: session_manager.get_session_count() == 1)

        with client.websocket_connect(f"/relay/{SESSION}?token={laptop_grant}") as laptop:
...
def test_third_connection_is_refused(client: TestClient) -> None:
    ...
    with client.websocket_connect(f"/relay/{SESSION}?token={browser_grant}"):
        with client.websocket_connect(f"/relay/{SESSION}?token={laptop_grant}"):
            with client.websocket_connect(f"/relay/{SESSION}?token={third_grant}") as third:
                expect_closed(third)
...
def test_disconnect_while_waiting_releases_the_session(client: TestClient) -> None:
    ...
    with client.websocket_connect(f"/relay/{SESSION}?token={grant}"):
        assert wait_until(lambda: session_manager.get_session_count() == 1)
...
def test_total_session_cap_is_enforced(
    ...
    with client.websocket_connect(f"/relay/session-one?token={first}"):
        assert wait_until(lambda: session_manager.get_session_count() == 1)

        with client.websocket_connect(f"/relay/session-two?token={second}") as extra:
            expect_closed(extra)
...
def test_per_user_session_cap_is_enforced(
    ...
    with client.websocket_connect(f"/relay/user-session-one?token={first}"):
        assert wait_until(lambda: session_manager.get_session_count() == 1)

        with client.websocket_connect(f"/relay/user-session-two?token={second}") as extra:
            expect_closed(extra)

        # A different account is unaffected by the first account's usage.
        with client.websocket_connect(f"/relay/other-user-session?token={other_user}"):
